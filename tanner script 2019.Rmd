...
### Build the plot
```{r fig1plot, exercise=F, warning = F, message = F, eval = T, exercise.timelimit = 10000}
...
## Figure 1 sip duration {.tabset}
```{r, warning = F, message = F, eval = T, exercise.timelimit = 10000}

calc_feeding_time("http://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%20100.100%20vs%20100.00%20+Date.xls?raw=true")

## this original one doesn't split dieta from dietb and i think it only calculates feeding time on dieta. 
calc_feeding_time2 <- function(gh_filepath) {
...
print(kruskal.test(pd2$time ~ pd2$trt))

}

## newer
calc_feeding_time <- function(gh_filepath,j=3) {
GET(gh_filepath, write_disk(tf <- tempfile(fileext = ".xls")))
...
pref_data$tt <- with(pref_data, interaction(time_consuming_diet, trt))

print(kruskal.test(pref_data$value ~ pref_data$tt))

}

gh_filepath <- "http://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%20100.100%20vs%20100.00%20+Date.xls?raw=true"
...
calc_feeding_time("http://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%20100.100%20vs%20100.00%20+Date.xls?raw=true")
# plot 75y100g
calc_feeding_time("https://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%20100g75y%20+Date.xls?raw=true")
### 50g100y
calc_feeding_time("https://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%2050g100y%20+Date.xls?raw=true")
### 25g100y
calc_feeding_time("https://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%2025g100y%20+Date.xls?raw=true")
### 75g100y
calc_feeding_time("https://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%2075g100y%20+Date.xls?raw=true")
### 100g50y
calc_feeding_time("https://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%20100g50y%20+Data.xls?raw=true")
### 100g25y
calc_feeding_time("https://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%20100g25y%20+Date.xls?raw=true")
### 50g50y
calc_feeding_time("https://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%20100g100y%20vs%2050g50y%20+Date_JMC.xls?raw=true")
  #Error - use a file that sets 'sheet SipDurations and numberofSips cells c8 and e8 from empty to 0'
  calc_feeding_time("https://github.com/johnchaston/TBCdietPref/blob/9f1a4cbcf2aa633540f6fbb86aed351935c62583/files/DataInExcelFormat%20100g100y%20vs%2050g50y%20+Date_JMC.xls?raw=true")
### 50g25y
calc_feeding_time("https://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%20100.100%20to%2050.25+Date_JMC.xls?raw=true")
...
### unknown???
```{r fig150g100y, warning=F, message = F, eval = T, exercise.timelimit=10000}
GET("http://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%20100.100%20vs%20100.00%20+Date.xls?raw=true", write_disk(tf <- tempfile(fileext = ".xls")))
...
for(i in 1:j) {
	...
	lp2 <- cbind(lacto_pref,abb[,(i)], aab[,i],aab[,(i+j)],aaa[,i],aaa[,(j+i)])
	lp3 <- lp2[(!is.na(lp2[,c(1)]) & lp2[,c(1)]!=-1 & !is.na(lp2[,c(3)]) & lp2[,c(3)]!=-1 & !is.na(lp2[,c(4)]) & lp2[,c(4)]!=-1),]
	lp4 <- data.frame(lp3) %>% mutate(trt=paste0(colnames(ccc[i]),"_",colnames(ccc[i+j])), pref=lp3[,1], day=lp3[,2], time1=lp3[,3], time2 =lp3[,4],dieta=lp3[,5], dietb = lp3[,6]) %>% dplyr::select(trt,pref,day,time1, time2,dieta,dietb) %>% mutate(diettimea = dieta * time1, diettimeb = dietb * time2) %>% dplyr::select(-time1, -time2, -dieta, -dietb) %>% dplyr::rename(dieta = diettimea, dietb = diettimeb)
	out_df <- rbind(out_df,lp4)
}
...
shapiro.test((pref_data$value)^(1/4))
hist((pref_data$value^(1/4)))
pref_data$transformed <- (pref_data$value)^(1/4)

pref_data$tt <- with(pref_data, interaction(time_consuming_diet, trt))

abc <- lmer(transformed ~ time_consuming_diet * trt + (1|day), pref_data)
anova(abc)
abc <- lmer(transformed ~ tt + (1|day), pref_data)
anova(abc)
abc2 <- glht(abc, mcp(tt = "Tukey"))
abc3 <- cld(abc2)
plots(abc3)


abc <- lmer(value ~ time_consuming_diet * trt + (1|day), pref_data)
anova(abc)
abc <- lmer(value ~ tt + (1|day), pref_data)
anova(abc)
abc2 <- glht(abc, mcp(tt = "Tukey"))
abc3 <- cld(abc2)
plot(abc3)
...
ggplot(pref_data, aes(x = tt, y = transformed)) + geom_point()
data.frame(abc3$lp, abc3$x)
...
pref_data3 <- out_df %>% 
  ...
  mutate(total = dieta+dietb) %>%
	droplevels() %>%
  mutate(value = dieta/(dieta+dietb))

shapiro.test((pref_data3$value)^(1))
shapiro.test(log10(pref_data3$value))
hist((pref_data3$value^(1)))
pref_data$transformed <- (pref_data3$value)^(1/4)

abc <- lmer(value ~ trt + (1|day), pref_data3)
anova(abc)
abc2 <- glht(abc, mcp(trt = "Tukey"))
abc3 <- cld(abc2)
plot(abc3)
abc3
...
## figure 1 total feeding preference index
```{r fig150g100y, warning=F, message = F, eval = T, exercise.timelimit=10000}

total_pref_index <- function(gh_filepath) {
...
print(shapiro.test((pref_data3$value)^(1)))
print(shapiro.test((pref_data3$value)^(1/2)))
print(shapiro.test((pref_data3$value)^(1/3)))
shapiro.test(log10(pref_data3$value))
hist((pref_data3$value^(1)))
pref_data3$transformed <- (pref_data3$value)^(1/2)
...
# plot 100g100y
total_pref_index("http://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%20100.100%20vs%20100.00%20+Date.xls?raw=true")
## not normal - aaa

# plot 75y100g
total_pref_index("https://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%20100g75y%20+Date.xls?raw=true")
## close to normal - aaa

### 50g100y
total_pref_index("https://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%2050g100y%20+Date.xls?raw=true")
## not normal - aaa

### 25g100y
total_pref_index("https://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%2025g100y%20+Date.xls?raw=true")
# not normal - aaa

### 75g100y
total_pref_index("https://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%2075g100y%20+Date.xls?raw=true")
# not normal - aaa

### 100g50y
total_pref_index("https://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%20100g50y%20+Data.xls?raw=true")
# close to normal - a ab b 

### 100g25y
total_pref_index("https://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%20100g25y%20+Date.xls?raw=true")
# close to normal - aaa

### 50g50y
total_pref_index("https://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%20100g100y%20vs%2050g50y%20+Date_JMC.xls?raw=true")
  #Error - use a file that sets 'sheet SipDurations and numberofSips cells c8 and e8 from empty to 0'
  total_pref_index("https://github.com/johnchaston/TBCdietPref/blob/9f1a4cbcf2aa633540f6fbb86aed351935c62583/files/DataInExcelFormat%20100g100y%20vs%2050g50y%20+Date_JMC.xls?raw=true")
# close to normal - aaa
  
### 50g25y
total_pref_index("https://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat%20100.100%20to%2050.25+Date_JMC.xls?raw=true")
# close to normal - aaa
```
...
# 
# 
# gh_filepath <- "https://github.com/johnchaston/TBCdietPref/blob/6518428a01fa32331c67e05ef101baf28fc14ff7/files/DataInExcelFormat-2JMC.xls?raw=true"
j=41
...
out_df <- data.frame(trt=character(), pref=numeric(),day=numeric(),time=numeric(),dieta=character(),dietb=character())
 ...
 for(i in 1:j) {
  ...
  #	filter(dieta<200 & dietb<200) %>%
# "103 100g100y_103 100g50y","71 100g100y_71 100g50y" are dropped because they weren't in the lookup file
  pref_data <- out_df %>% 
  	filter(sip1<1000 & sip2<1000) %>%
...
kwtest <- kruskal.test(pref_data$value ~ pref_data$tt)
  ...
  dim(pref_data)
  pref_data[1,]
  anova(lm(value ~ time_consuming_diet * trt2, pref_data))
  if (kwtest$p.value < 0.05) {
    efg <- dunn.test(pref_data$value, pref_data$tt, method = "bh", table = F, kw=F)
    ghi <- cldList(comparison = efg$comparisons, p.value = efg$P.adjusted, threshold = 0.05)
    diff_codes <- as.character(unlist(ghi$Letter))
...
## doesn't run to avoid specifying each individual folder path, but can download from github files folder and program the folder locally.
...
setwd('../..')
WriteMCL(joined_matrixB,"wilcox_binomial_10_20_now2021.csv")
...
mcl_matrixE <- AnalyzeOrthoMCL(mcl_data = parsed_data, 
						pheno_data = pd3,
						 model = 'binom',
						 species_name = 'mgwa2',
						 resp_var1 = 'dieta',
...
species_name = 'code'; rndm1='day2'; rndm2='time2'; princ_coord = 0.5
mcl_data=parsed_data
pheno_data=pd2
model = 'binom', 
species_name = 'code',  
resp1 = 'dieta',
...
setwd('~/Dropbox/Dietary Pref 2019/MGWA/')
joined_matrixD <- JoinRepSeq(parsed_data, 'mgwa2/precompliantFasta3/', mtrx, fastaformat = 'old')
setwd('~/Dropbox/Dietary Pref 2019/MGWA/')
WriteMCL(joined_matrixD,"fullbinomial_12_13_2021.csv")
QQPlotter(mtrx)
...
## Figure 3 totals - add date?
```{r fig3, exercise=T, warning = F, message = F, eval = T, fig.width = 5, exercise.timelimit = 10000 }

## calc_feeding_time_stats("https://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat-all%20together%20with%20dayandtime.xls?raw=true", j=8)
...
#ggsave(h=7,w=9,"mutant_validation_totals-2_27.png")
x=.6
#ggsave(h=5.17*x,w=6.64*x,"mutant_validation_totals-2_27.png")

```

## Figure 3 sip duration
```{r}
...
j=16
# 
# gh_filepath <- "https://github.com/johnchaston/TBCdietPref/blob/master/files/DataInExcelFormat.xls?raw=true"
# j=8
# GET(gh_filepath, write_disk(tf <- tempfile(fileext = ".xls")))
# 
# aaa <- read_xls(tf, sheet = "NumberOfSips")
...
#ggsave(h=7,w=9,"fig3-sipduration.png")
ggplot(pd3e, aes(x = abc2.x, xend=abc2.x, y=c(rep(0.5,length(table(list(pd3e$abc2.x))))), yend=mean)) + 
	geom_segment(stat="identity", size = 8,col="red") + #size=24, 
	scale_fill_manual(name="Legend",values=colors2) +
	scale_y_continuous(limits = c(0,250))+
